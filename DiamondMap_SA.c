...
DrawTile_SA(SDL_Surface *s, int n, int sx, int sy)
{
	...
	if (k > 0) {
		...
		if (t != NULL) {
			XSurface x;
			memset(&x, 0, sizeof(XSurface));

			XSurface_Set(&x, t->Width, t->Height, 8, 0x3, 0, Palette_Get(0));

			ret = !Graph_Decompress(
				&x,
				t->BlockLength,
				Graphic_Get(t->GraphOffset)+sizeof(GraphHeader)
			);

			if (ret) {
				SDL_Surface *tm;
				SDL_Rect r;
				tm = (SDL_Surface *)XSurface_ConvertToSDLSurface(&x);
				if (tm != NULL) {
					r.x = sx + t->dx; r.y = sy + t->dy;
					ret = !SDL_BlitSurface(tm, NULL, s, &r);
					SDL_FreeSurface(tm);
				}
			}
